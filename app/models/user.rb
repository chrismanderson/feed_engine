class User < ActiveRecord::Base
  ...
  validates :display_name, :format => { :with => /\A[a-zA-Z0-9_-]+\z/,
            :message => "may only contain letters, numbers,
  ...
  STREAMABLE_TYPES = ["ImageItem","LinkItem","TextItem"]
  ...
  def last_retrout_id_for_user(followed_user)
    items = stream_items.where(:streamable_type => STREAMABLE_TYPES).
                    where(:original_author_id => followed_user.id)
    if items.any?
      items.last.id
    else
      1
  ...
  def has_retrouts_for?(item)
    stream_items.where(:refeed => true).where(:streamable_type => item.class).
    where(:streamable_id => item.id).where(:refeed => true).any?
  end
