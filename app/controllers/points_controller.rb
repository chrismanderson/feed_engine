class PointsController < ApplicationController
  def create
    if current_user
      @point = Point.new(user: current_user,
                 pointable_id: params[:item_id],
               pointable_type: params[:item_type])
      if @point.save
        session[:point_for] = nil
        redirect_to request.referrer,
          :notice => "You made some points. Rock on!"
      else
        redirect_to request.referrer,
          :alert => "You've already pointed this item"
      end
    else
